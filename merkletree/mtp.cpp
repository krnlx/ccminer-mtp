...
MerkleTree::Elements   mtp_init2(argon2_instance_t *instance, int thr_id) {
...
//printf("size %d\n", instance->memory_blocks);
	...
	for (int i = 0; i < instance->memory_blocks ; ++i) {
		uint8_t *digest;
		digest = &mem[MERKLE_TREE_ELEMENT_SIZE_B*i];
		elements.emplace_back(digest, digest + MERKLE_TREE_ELEMENT_SIZE_B);
	}
	free(mem);
//for(;;);
	return elements;
	...
	if (instance != NULL) {
		...
		for (int i = 0; i < instance->memory_blocks / 2; ++i) {
                        uint64_t *mr=(uint64_t *)digest;
			//printf("sz %d nr %d\n", MERKLE_TREE_ELEMENT_SIZE_B, instance->memory_blocks);
//                        if(i==0)
  //                              printf("candidate %lx %lx %lx\n",mr[0],mr[1],instance->memory[2 * i].v[0]);
