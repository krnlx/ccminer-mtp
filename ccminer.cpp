...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (!sctx->job.job_id) {
		applog(LOG_WARNING, "stratum_gen_work: job not yet retrieved %d",fucken_pool_counter);
		fucken_pool_counter++;
		if(fucken_pool_counter>30)
			exit(-1);
//		exit(-1);
		return false;
	}
...
//		printf("\n");
	...
	switch (opt_algo) {
		case ALGO_MTP:
//                                if(!sctx->next_target){
//                                applog(LOG_WARNING, "stratum_gen_work: empty target");
//                                return false;
//                                }else{
//                                        applog(LOG_WARNING, "stratum_gen_work: good target");
//                                }
...
//						proper_exit(EXIT_CODE_CUDA_NODEVICE);
			...
			applog(LOG_ERR, "%d devices online", real_d);
			if(!real_d){
				applog(LOG_ERR, "Non-existant CUDA devices specified");
				proper_exit(EXIT_CODE_CUDA_NODEVICE);
			}
...
int main(int argc, char *argv[])
{
	...
	printf("*** ccminer " PACKAGE_VERSION " for nVidia GPUs by djm34 & krnlx***\n");
#ifdef _MSC_VER
	...
	printf("  *** News (07/06/2018): MTP algo for ZCoin \n\n");
        printf("  *** News (28/12/2018): MTP algo for ZCoin improved by krnlx \n\n");
	...
	printf("ZCoin donation address: aChWVb8CpgajadpLmiwDZvZaKizQgHxfh5 (djm34)\n\n");
        printf("BTC donation address: 1FJGCKYw1pDamUxQS1dNTTVNVpWsCjPLUd (krnlx)\n");
        printf("ZCoin donation address: aFZGwPK5dAb8i4feQ5WjRjvWhFiMQn8JwZ (krnlx)\n\n");
